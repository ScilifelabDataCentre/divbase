...
This script takes the task IDs from the json file and fetches their runtime information from the celery results backend. The factors (samples, variants) and response (runtime) are then visualised and an
...
from _benchmarking_shared_utils import DIVBASE_API_URL, get_divbase_access_token
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Analyse the results (runtime) of a factorial design run in divbase.")
    parser.add_argument(
        "--input-json",
        type=str,
...
def fetch_task_from_api(task_id: str, access_token: str) -> dict:
    """Fetch a single task's details from the DivBase API."""
    headers = {"Authorization": f"Bearer {access_token}"}
    response = requests.get(f"{DIVBASE_API_URL}/task-history/id/{task_id}", headers=headers, timeout=10)
...
def get_runtime_of_succeeded_tasks(task_records: list[dict], output_path: Path) -> list[dict]:
    """
    Use the celery results backend to get the runtime of the task IDs in the input JSON file.
    ...
    # TODO write somewhere that this assumes that all tasks have finished and that they have been sucessful. maybe print a message about which records in the json that are missing
    access_token = get_divbase_access_token()

    for record in task_records:
        task_id = record["task_id"]
        try:
            task_info = fetch_task_from_api(task_id, access_token)
            ...
            print(f"Task {task_id}: Failed to fetch from API - {e}")
            record["runtime"] = None
...
def plot_3D_response_surface(df: pd.DataFrame, query_str: str) -> None:
    ...
    if len(n_replicates_set) == 1:
        ...
        n_replicates_str = f"varies ({min(n_replicates_set)}–{max(n_replicates_set)})"
...
def main():
    ...
    if all_records_have_runtime:
        ...
        print("All records already have runtime and elapsed_time. Skipping API fetch.")
    else:
