...
class BcftoolsQueryManager:
    ...
    def run_current_command(self, cmd_config: dict[str, Any]) -> tuple[list[str], dict[str, float]]:
        ...
        for f_counter, file in enumerate(input_files):
            ...
            # Add --threads parameter for parallel compression/decompression (uses multiple cores)
            formatted_cmd = f"{cmd_with_samples} {file} --threads 4 -Oz -o {temp_file}"
    ...
    def merge_or_concat_bcftools_temp_files(self, output_temp_files: list[str], identifier: str) -> str:
        ...
        if len(output_temp_files) > 1:
            if non_overlapping_sample_names:
                ...
                merge_command = (
                    f"merge --force-samples --threads 4 -Oz -o {unsorted_output_file} {' '.join(output_temp_files)}"
                )
                ...
                for sample_set, files in sample_set_to_files.items():
                    ...
                    if len(files) > 1:
                        ...
                        concat_command = f"concat --threads 4 -Oz -o {concat_temp} {' '.join(files)}"
                ...
                if len(temp_concat_files) > 1:
                    merge_command = (
                        f"merge --force-samples --threads 4 -Oz -o {unsorted_output_file} {' '.join(temp_concat_files)}"
                    )
                    proc = self.run_bcftools(command=merge_command)
                    proc.wait()
        ...
        annotate_command = f"annotate -h {divbase_header_for_vcf} --threads 4 -Oz -o {annotated_unsorted_output_file} {unsorted_output_file}"
        proc = self.run_bcftools(command=annotate_command)
        proc.wait()

        sort_command = f"sort --threads 4 -Oz -o {output_file} {annotated_unsorted_output_file}"
