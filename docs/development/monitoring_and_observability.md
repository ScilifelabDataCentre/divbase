...
A per-task resource monitoring system for the Celery workers is implemented using a custom `prometheus-client` metrics server and `psutil`. For details on this see [Monitoring: Celery Worker Metrics](worker_metrics.md).

### RabbitMQ queue

Scraped using the built-in Prometheus plugin that comes with RabbitMQ `management-alpine` image
