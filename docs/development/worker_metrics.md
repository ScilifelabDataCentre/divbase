...
In the current DivBase implementation, bcftools subprocesses run sequentially and not in parallel. Each subprocess completes before the next one starts (see `run_current_command()` in `packages/divbase-api/src/divbase_api/services/vcf_queries.py`). This means that **allocating more CPU cores will NOT reduce wall time** in the current implementation.
