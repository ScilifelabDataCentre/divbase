...
Tasks can be divided by how they are submitted to the queue: user-submitted tasks, and system-submitted periodic tasks. User-submitted tasks are manually enqueued in the job management system by entering a command in the DivBase CLI. System-submitted periodic tasks are cronjobs that are enqueued based on a time schedule, and typically handle system maintenance tasks such as cleanup jobs.
...
!!! warning "Task not re-enqueued if workers crash or restart"
    ...
    Also worth noting is that the failed task is not invisible. `CeleryTaskMeta` will record the failure and `divbase-cli task-history` will show the user that their task did not complete.
...
- Two Celery signal handlers are used to write to additional PostgreSQL tables to capture values that are not included in `CeleryTaskMeta`. Like the Celery app, these should not be altered unless they are part of a refactoring.
  - The `after_task_publish` signal handler handles cron jobs specifically. This signal fires when a task is published to the queue. If the task name begins with `cron_tasks.` an entry is written to `TaskHistoryDB`. For user-submitted tasks, the writes to `TaskHistoryDB` are handled by the API, as described below in Section 2.3.
...
- Dynamic task routing is optional
...
- Tasks should also call the `check_queue_closed_for_new_tasks` fn to validate the queue is not closed for planned maintenance. This function raises a `QueueClosedError` if the queue is closed, which is handled in the API layer to return a 400 error with the `reason_for_users` message. See the [Closing and re-opening the Celery queue](deployment.md#closing-and-re-opening-the-celery-queue) docs for more details.
