...
!!! info "Terminology: Celery Task UUID vs. DivBase Task ID"
    Two distinct IDs are used in the DivBase task system:

    - **Celery Task UUID** — the UUID generated by Celery when a task is enqueued (e.g. the value of `result.id` after calling `.apply_async()`). This is an internal identifier stored in `CeleryTaskMeta` and `TaskHistoryDB`. It is never exposed to users.
    - **DivBase Task ID** — the auto-incrementing integer primary key from `TaskHistoryDB`, created by `create_task_history_entry()`. This is the ID returned to the user and displayed in the terminal after job submission. Users reference it with commands such as `divbase-cli task-history id <TASK_ID>`.

    **Disclaimer/TODO:**

    At the time of writing, the variable names in the current codebase does not yet fully reflect the above terminology (but the docs and user facing messages do). In the `divbase-api` package, the variable name `task_id` is frequently used for the **Celery Task UUID** (e.g. model columns, signal handler parameters, and the `task_id = <task_function>.request.id` pattern inside task bodies), while the **DivBase Task ID** integer is often named `job_id` (e.g. in API route functions, the `BcftoolsQueryKwargs` schema, and worker task parameters). This is a historical naming inconsistency that would benefit from a refactoring.
...
- Call the `create_task_history_entry()` CRUD function to record the Celery task UUID in `TaskHistoryDB` along with user and project ID. This function will return the DivBase Task ID, which is the autoincrementing integer primary key from the PostgreSQL table. It is much easier for users to handle than a full UUID.
- Finally, return the DivBase Task ID to the user client as an API response.
...
async def submit_vcf_query_job_endpoint(
    ...
    # the DivBase Task ID: an integer that is much less unwieldy than the full Celery Task UUID.
    task_id = await create_task_history_entry(
    ...
    # As an API response, return the DivBase Task ID to the CLI client that made the request, where it can be displayed to the user.
    return task_id
```
...
def vcf_query(
    ...
    # Using the pattern described in Section 2.3, the API returns the DivBase Task ID (an integer),
    # which can be displayed to the user in their terminal like such:
