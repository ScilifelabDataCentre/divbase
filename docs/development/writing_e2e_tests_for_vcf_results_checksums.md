...
### 1.1. General pattern to calculate the double-hash headerless MD5 checksum for a results VCF file
...
When the job is finished, download the results file to disk and run the code in [Section 1.1](#11-general-pattern-to-calculate-the-double-hash-headerless-md5-checksum-for-a-results-vcf-file) in your terminal.
...
- `expected_checksum`: the expected `##` headerless checksum calculated as described in [Section 1.](#1-calculate-the-expected-double-hash-headerless-checksum) of this guide.
